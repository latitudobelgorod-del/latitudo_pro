...
<head>
    ...
    <? // Подтверждение прав в Яндекс.Вебмастере. В корне лежит ещё и файл-подтверждение
       // yandex_386938e47a83754f.html с тем же кодом (он вне git намеренно) — два способа
       // не конфликтуют. Метатег дополнительно закрывает городские поддомены: они делят
       // с основным доменом один DOCUMENT_ROOT, но в Вебмастере это отдельные сайты. ?>
    <meta name="yandex-verification" content="386938e47a83754f" />
    ...
    <? // Яндекс.Метрика №110963911. ДВА УРОВНЯ СБОРА по трём состояниям согласия.
       //
       // Условие первое, общее: только боевой домен latitudo.pro — с локалки не шлём
       // мусор в статистику.
       //
       // Дальше решает кука latitudo_cookie_consent (ставит баннер, см.
       // local/php_interface/include/cookie-banner.php). Проверка серверная, по куке из
       // запроса: гейт на JavaScript тут не годится, он не остановил бы <noscript>-пиксель.
       //
       //   '1' — согласие:  ПОЛНЫЙ сбор, с вебвизором и картой кликов.
       //   ''  — не выбрал: БАЗОВЫЙ сбор — визиты и цели, БЕЗ вебвизора и карты кликов.
       //   '0' — отказ:     счётчик не грузится нигде и никогда.
       //
       // ПОЧЕМУ ИМЕННО ТАК. Раньше счётчик грузился только на '1', и это стоило почти всей
       // статистики: посетитель, который баннер просто не заметил, не попадал в отчёты ни
       // визитом, ни целью — заявка приходила в CRM «из ниоткуда». Cookieless-режима у
       // Метрики нет (в параметрах init только webvisor/clickmap/trackLinks/
       // accurateTrackBounce), поэтому «считать, но не ставить cookie» невозможно.
       //
       // Граница между уровнями проведена по тяжести сбора, а не по удобству. Вебвизор
       // записывает сессию целиком, включая ввод в поля форм, — это данные, которые без
       // спроса собирать нельзя ни при каком толковании. Визит и факт достижения цели —
       // несопоставимо меньше. По букве 152-ФЗ ст. 9 _ym_uid всё равно персональные
       // данные и согласие нужно до его установки; решение о базовом уровне принято
       // владельцем сайта осознанно, как компромисс. Не решение юриста, а решение
       // заказчика — если политика изменится, менять надо здесь.
       //
       // ЯВНЫЙ ОТКАЗ НЕ ОБХОДИМ. На '0' счётчика нет, офлайн-конверсии в Метрику для
       // таких заявок тоже не досылаются (см. include/metrika-conversions.php).
       //
       // ⚠️ Вебвизор после нажатия «Принять» включается СО СЛЕДУЮЩЕЙ страницы: повторный
       // ym(id,'init') с другими параметрами Метрика игнорирует, апгрейд на живой странице
       // невозможен. Это осознанная потеря — ради неё не стоит перезагружать страницу
       // человеку, который только что согласился.
       //
       // Композитный кэш Битрикса выключен (~param_composite = N) — иначе страница с уже
       // вставленным счётчиком могла бы уехать из кэша отказавшемуся.
       //
    ...
    <? $isProd  = strpos((string)($_SERVER['HTTP_HOST'] ?? ''), 'latitudo.pro') !== false;
       $consent = (string)($_COOKIE['latitudo_cookie_consent'] ?? ''); // '' | '1' | '0'
       $metrikaOn   = $isProd && $consent !== '0'; // грузить счётчик вообще
       $metrikaFull = $consent === '1';            // вебвизор и карта кликов ?>

    <? // Состояние согласия для клиентского кода. Нужно и целям Метрики (не отправлять
       // их, если человек нажал «Отклонить» уже на этой странице — счётчик-то в разметке
       // остался), и форме заявки: она передаёт состояние на сервер, чтобы тот понимал,
       // сработала ли JS-цель. Объявлено безусловно, вне проверки $isProd: на локалке
       // форма пользуется этой же функцией. ?>
    <script>
    window.latitudoConsent = function () {
        var m = document.cookie.match(/(?:^|;\s*)latitudo_cookie_consent=([01])/);
        return m ? m[1] : '';
    };
    /* Можно ли отправлять данные в Метрику ПРЯМО СЕЙЧАС. Отличается от наличия window.ym:
       счётчик мог загрузиться на базовом уровне, а посетитель — нажать «Отклонить» уже
       после этого. Выгрузить Метрику из страницы нельзя, поэтому цели проверяют это сами. */
    window.latitudoMetrikaAllowed = function () { return window.latitudoConsent() !== '0'; };
    /* Загрузился ли tag.js ФАКТИЧЕСКИ. Наличия window.ym для этого недостаточно: загрузчик
       выше определяет заглушку ym синхронно, и она существует даже когда mc.yandex.ru
    ...
    <? // Envybox «Обратный звонок» (виджет cbk, код сайта 4752cf60…). Кнопка в углу экрана:
       ...
       //
       // ПОЧЕМУ БЕЗ ГЕЙТА ПО СОГЛАСИЮ, в отличие от Метрики. Это функциональный элемент,
       // как виджет Яндекс.Карт в подвале: посетитель сам нажимает кнопку и сам вводит
       // телефон. Метрика же пишет сессию пассивно (вебвизор — вплоть до ввода в поля),
       // о чём человека не спрашивали. Спрятать кнопку заказа звонка за баннером cookie
       // означало бы, что отказавшийся (и любой, кто ещё не выбрал) просто не увидит
       // способа связаться — это потеря заявок на ровном месте.
       // Захотите ужесточить — добавьте в условие ниже && $metrikaFull, переменная уже есть.
       //
       // Только боевой домен — как у Метрики: с локалки не накручиваем визиты
       // в кабинете Envybox и не дёргаем менеджеров тестовыми заявками.
       //
       // Ссылки на CDN оставлены как выдал сервис: cdn.envybox.io — российский хостинг,
    ...
    <? // Цель Метрики на заказ звонка. Envybox сам вызывает эти два глобальных обработчика:
       ...
       //
       // Две проверки перед отправкой цели, и ни одна не перестраховка:
       //   window.ym — счётчика может не быть вовсе (отказ от cookie), без guard'а тут
       //               был бы ReferenceError на каждой заявке;
...
<body>
    <? // Пиксель для браузеров без JS — под тем же условием, что и счётчик ($metrikaOn),
       // то есть на отказе его нет. Уровень сбора тут не при чём: пиксель фиксирует
       // только визит, вебвизора и карты кликов без JS всё равно не бывает.
            ...
            <div class="topbar__inner">
                ...
                <div class="topbar__left">
                    ...
                    <span class="topbar__item topbar__item--address">
                        ...
                        <span class="topbar__text">#REGION_ADDRESS#</span>
                    </span>
                    <? endif ?>
                </div>
                <div class="topbar__right">
                    <? if ($store && $store['EMAIL'] !== ''): ?>
                    <a class="topbar__item" href="mailto:#REGION_EMAIL#">
                        ...
                        <span class="topbar__text">#REGION_EMAIL#</span>
                    </a>
                    <? endif ?>
                    ...
                    <span class="topbar__item">
                        ...
                        <span class="topbar__text">#REGION_WORK_HOURS#</span>
                    </span>
                    <? endif ?>
                    <? if ($store && $store['PHONE'] !== ''): ?>
                    <a class="topbar__item topbar__item--phone" href="#REGION_PHONE_HREF#">
                        ...
                        <span class="topbar__text">#REGION_PHONE#</span>
                    </a>
                    <? endif ?>
                </div>
            </div>
