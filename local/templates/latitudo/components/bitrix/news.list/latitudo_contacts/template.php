...
// Строковые свойства магазина текущего региона
...
// Карта: в свойстве может лежать готовый <iframe> или просто URL
$cMapRaw  = trim((string)($arItem["PROPERTIES"]["MAP_EMBED"]["VALUE"] ?? ''));
$cMapHtml = '';
if ($cMapRaw !== '') {
    $cMapHtml = (mb_stripos($cMapRaw, '<iframe') !== false)
        ? $cMapRaw // готовый embed-код — выводим как есть
...
// Мессенджеры — вшиты в шаблон, одни и те же во всех регионах (ссылки заменит заказчик)
$cMessengers = [
