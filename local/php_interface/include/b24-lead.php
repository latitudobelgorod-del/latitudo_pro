...
function latitudoB24BuildLeadFields(array $post, string $title, int $assignedId): array
{
    ...
    $fields = [
        ...
        // Адрес страницы уходит в ДВА поля, это не дубль по недосмотру:
        //   UF_CRM_1670914240846 «Страница заявки» — тип url, кликабельная ссылка в карточке;
        //   UF_CRM_1670914333154 «Домен сайта»     — тип string, по нему в Б24 построены
        //                                            старые отчёты и фильтры.
        // Во второе исторически кладётся полный адрес, а не домен — поле названо одно,
        // а хранит другое. Менять на голый хост в одиночку нельзя: сломаются сохранённые
        // фильтры у отдела продаж, это отдельное согласованное изменение.
        'UF_CRM_1670914240846' => $get('b24_page_url'),
