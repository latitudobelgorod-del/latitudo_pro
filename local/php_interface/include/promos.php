...
function latitudoPromoTermsHtml(string $text, string $type = ''): string
{
    ...
    // Настоящая блочная разметка от редактора — она главнее наших догадок.
    // <br> сюда НЕ входит намеренно, см. следующий абзац.
    if (preg_match('~<(?:p|ul|ol|li|div|h[1-6]|table)\b~i', $text)) {
        return $text;
    }

    // Поле типа «текст» Битрикс отдаёт уже переработанным в HTML: сам расставляет
    // <br /> по переносам и экранирует символы (скобки → &#40; &#41;). Из-за этого
    // первая версия функции считала такой текст авторской разметкой и не трогала его —
    // акции с типом text оставались строчками без абзацев и с литеральными «•», пока
    // соседние (тип html) выглядели нормально. Поэтому <br> разбираем как обычный
    // перенос строки и строим структуру дальше.
    // Съедаем и перенос СРАЗУ ЗА тегом: Битрикс выводит «<br />\n», и без этого каждая
    // строка отделялась бы пустой строкой, а пустая строка закрывает список — все девять
    // пунктов превращались в девять списков по одному пункту. Два <br> подряд по-прежнему
    // дают пустую строку, то есть осознанный разрыв редактора сохраняется.
    $hasBr = (bool)preg_match('~<br\s*/?>~i', $text);
    $text  = preg_replace('~<br\s*/?>[ \t]*\r?\n?~i', "\n", $text);

    // Экранируем только сырой текст. Если Битрикс уже прошёлся по полю (есть <br>)
    // или поле объявлено html — сущности готовые, второе экранирование превратило бы
    // «&#40;» в видимое «&amp;#40;».
    $isHtml = $hasBr || strtolower($type) === 'html';
