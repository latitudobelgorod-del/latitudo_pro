...
$arUrlRewrite=array (
  ...
  // robots.txt и sitemap.xml отдаются кодом, файлов на диске нет: обоим нужна строка
  // с АБСОЛЮТНЫМ адресом текущего хоста, а docroot у голого домена и пяти поддоменов
  // филиалов общий (см. комментарии в самих роутах). Правила стоят до общего правила
  // лендингов — оно односегментное и точку в адресе не ловит, но порядок важнее
  // случайного совпадения: движок идёт по массиву сверху вниз.
  6 =>
  array (
    'CONDITION' => '#^/robots\\.txt(\\?.*)?$#',
    'RULE' => '',
    'ID' => NULL,
    'PATH' => '/local/routes/robots.php',
    'SORT' => 150,
  ),
  7 =>
  array (
