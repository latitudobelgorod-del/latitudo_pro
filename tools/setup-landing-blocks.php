...
$fields = [
    ...
    [
        'ENTITY_ID'         => $ufEntity,
        'FIELD_NAME'        => 'UF_SHOW_DEALERS',
        'USER_TYPE_ID'      => 'boolean',
        'MULTIPLE'          => 'N',
        'MANDATORY'         => 'N',
        'SORT'              => 840,
        // DEFAULT_VALUE = 1: блок сквозной и до появления галочки был на всех страницах,
        // поэтому новый раздел показывает его по умолчанию, а прячут уже вручную.
        'SETTINGS'          => ['DEFAULT_VALUE' => 1, 'DISPLAY' => 'CHECKBOX'],
        'EDIT_FORM_LABEL'   => ['ru' => 'Показывать блок «Дилерам и партнёрам»'],
        'LIST_COLUMN_LABEL' => ['ru' => 'Блок «Дилерам и партнёрам»'],
    ...
    // «Перилах» и «Фасадах»; на «Ступенях» (там включён «Как мы работаем») — сразу за
    // «Как мы работаем», перед «Отзывами». Галочкой, а не слагами в коде — чтобы так же
    // можно было сделать любому разделу из админки.
    [
        'ENTITY_ID'         => $ufEntity,
        ...
        'HELP_MESSAGE'      => ['ru' => 'Галочки нет — квиз стоит сразу под первым экраном, над «Акциями месяца» (как в макете). Галочка стоит — квиз переезжает вниз: сразу после блока «О компании», перед «Дилерам и партнёрам» (если он включён) или «Посетите магазин». Если у раздела включён блок «Как мы работаем» — сразу после него, перед «Отзывами». Сам квиз выбирается как раньше, в инфоблоке «Марквизы» по разделу и городу.'],
    ],
    ...
    // Заголовок заявки: идёт в тему письма и в название лида Б24 (+ домен в скобках,
    // см. include/b24-lead.php). Заведено вручную в админке прода; здесь — чтобы поле
    // появилось в локальной и любой новой базе. Значения (текст) заполняет контент-менеджер.
    [
        'ENTITY_ID'         => $ufEntity,
        'FIELD_NAME'        => 'UF_HEAD_ZAYAVKA',
        'USER_TYPE_ID'      => 'string',
        'MULTIPLE'          => 'N',
        'MANDATORY'         => 'N',
        'SORT'              => 840,
        'SETTINGS'          => ['SIZE' => 60, 'ROWS' => 1, 'MIN_LENGTH' => 0, 'MAX_LENGTH' => 0, 'DEFAULT_VALUE' => '', 'REGEXP' => ''],
        'EDIT_FORM_LABEL'   => ['ru' => 'Заголовок заявки (тема письма и название лида)'],
        'LIST_COLUMN_LABEL' => ['ru' => 'Заголовок заявки'],
        'LIST_FILTER_LABEL' => ['ru' => 'Заголовок заявки'],
        'HELP_MESSAGE'      => ['ru' => 'Текст идёт в тему письма и в название лида в Битрикс24; в скобках автоматически добавляется домен филиала. Например: «Заявка на перголы (rostov.latitudo.ru)». Пусто → «Заявка с сайта (домен)».'],
    ],
];
...
$layout = [
    ...
    'stupeni'             => ['UF_SHOW_ABOUT' => 0, 'UF_SHOW_HOW_WE_WORK' => 1, 'UF_SHOW_ON_MAIN_PAGE' => 1, 'UF_MARQUIZ_BOTTOM' => 1],
